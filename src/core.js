...
const NPC_DEFS = {
  water_keeper: {
    ...
    role: "水場の管理者",
    lines: [
      "水筒は持った？ 下では喉が渇いてからじゃ遅い。",
      "東の隔壁の先が廃棄層。まずは浅い区画で無理せず戻ってきて。",
      "本命は深部の浄水コア。今の濾過槽は、もう長くない。"
    ],
    afterRun: [
      "戻ってきたね。成果が少なくても、生きて帰ったなら十分。",
      "汚染を受けたまま粘るな。フィルタを使うか、帰るかを早めに決めて。",
      "次は水音の変わる階層を探して。古い浄水管が近い合図だ。"
    ],
    depth3: [
      "深度3まで行ったなら、中枢は遠くない。",
      "そこから先は、機械が人を障害物として処理する。正面から付き合うな。",
      "コアを見つけたら欲張らず戻って。集落にはそれだけで価値がある。"
    ],
    core: [
      "これで水はしばらく保つ。全員が今日を越せる。",
      "でも廃棄層は止まっていない。次の故障は必ず来る。",
      "休んで。次に潜る時も、帰ってくることを最優先にして。"
    ]
  },
  old_digger: {
    key: "old_digger",
    ...
    role: "引退した発掘家",
    lines: [
      "初めてなら、広い部屋で戦うな。通路へ下がれ。",
      "見えていない場所は信用するな。地図は歩いた分だけ本物になる。",
      "発掘家の仕事は拾うことじゃない。持って帰ることだ。"
    ],
    afterRun: [
      "一度戻ったなら、次は道を覚えている。そこが新人と死体の差だ。",
      "欲張るな。袋が重くなるほど、判断は鈍る。",
      "帰還は敗北じゃない。情報を持ち帰れば、集落は次に備えられる。"
    ],
    depth3: [
      "深く潜るほど、帰り道の価値が上がる。",
      "強い敵を倒すより、相手にしない道を選べ。",
      "中枢前で迷ったら戻れ。迷っている時点で準備が足りない。"
    ],
    core: [
      "コアを見たか。あれは宝じゃない。集落全員の命綱だ。",
      "お前は発掘家になった。まだ英雄にはならなくていい。",
      "次も同じだ。生きて戻れ。それだけ守れ。"
    ]
  },
  mechanic: {
    ...
    role: "工具と端末の整備担当",
    lines: [
      "下で拾う古い道具は、使い方を間違えるとただの重りだ。",
      "信号妨害機は囲まれてからじゃ遅い。囲まれそうな時に使え。",
      "強制転送端末は敵を倒す道具じゃない。時間を買う道具だ。"
    ],
    afterRun: [
      "拾ったものは名前より働きで覚えろ。逃げる道具か、止める道具か、壊す道具か。",
      "解体指示書は壁を壊せる。つまり、行き止まりを行き止まりじゃなくできる。",
      "緊急帰還タグはケチるな。死んだら修理も鑑定もできない。"
    ],
    depth3: [
      "軍事ロボットの射線に立つな。角を曲がれば、古い銃口もただの鉄くずだ。",
      "端末の近くは罠も多い。便利な場所ほど、誰かが守らせている。",
      "中枢の部品は規格が違う。触る前に周囲の機械を止めろ。"
    ],
    core: [
      "コアは生きている部品だ。雑に扱えば、集落ごと吹き飛ぶ。",
      "よく持ち帰った。これで濾過槽は直せる。完全じゃないが、十分だ。",
      "次に必要なのは予備部品だな。つまり、また潜ることになる。"
    ]
  },
  lookout: {
    key: "lookout",
    ...
    role: "入口の監視役",
    lines: [
      "東の隔壁が廃棄層入口だ。外から見れば静かだが、中は毎日組み替わる。",
      "清掃ロボットは人を汚染源と見る。悪意じゃない。だから止まらない。",
      "追われたら角を曲がれ。機械の目は強いが、万能じゃない。"
    ],
    afterRun: [
      "戻ったか。入口の音が少し変わった。下の構造がまた動いている。",
      "警備ロボットは見つけると速い。視線を切れば、追跡は長く続かない。",
      "物流ロボットは遺物を持っていく。見つけたら放置するな。"
    ],
    depth3: [
      "深部では警報線に気をつけろ。踏めば、こちらの位置を機械に教える。",
      "重い足音が聞こえたら距離を取れ。勝てる相手か確かめるな。",
      "帰り道に敵を残すな。戻る時の一歩が命取りになる。"
    ],
    core: [
      "中枢防衛機が止まった時、入口の振動も一瞬止まった。",
      "あれはただの敵じゃない。廃棄層そのものの番犬だ。",
      "今日は隔壁を閉める。お前は休め。"
    ]
  },
  recorder: {
    ...
    role: "発掘記録の管理者",
    lines: [
      "初回の目的を書いておく。東の入口から入り、危なくなる前に戻る。",
      "見つけたものは記録する。名前をつければ、次に出会った時に迷わない。",
      "浄水コアは深部目標。焦って取りに行くものではない。"
    ],
    afterRun: [
      "発掘記録を更新した。何を拾ったかより、どこで危なかったかが重要。",
      "死因を書けるのは、生きて帰った者だけだ。次の判断材料にする。",
      "同じ失敗を減らせば、集落の寿命は少し伸びる。"
    ],
    depth3: [
      "深度3以降の記録は少ない。戻った発掘家が少ないからだ。",
      "ここから先は、見たものを全部信用しないで。AIの区画は目的ごと変わる。",
      "戻ったら、道順を最初に話して。記録は次の命綱になる。"
    ],
    core: [
      "浄水コアの記録を残した。次世代が、ただの神話だと思わないように。",
      "今日の記録名は『水の音が戻った日』にする。異論は受け付けない。",
      "次は廃棄層の変化を追う。勝って終わり、という場所ではない。"
    ]
  }
};
...
function getNpcDialogue(game, key) {
  ...
  const runs = game.settlement?.runs || 0;
  const bestDepth = game.settlement?.bestDepth || 0;
  const cores = game.settlement?.cores || 0;
  let lines = [...(npc.lines || [])];

  if (cores > 0 && npc.core) {
    lines = [...npc.core];
  } else if (bestDepth >= 3 && npc.depth3) {
    lines = [...npc.depth3];
  } else if (runs > 0 && npc.afterRun) {
    lines = [...npc.afterRun];
  }

  const runLogs = Array.isArray(game.settlement?.runLogs) ? game.settlement.runLogs : [];
  if (key === "recorder" && runLogs.length && cores <= 0) {
    lines[2] = `直近の記録: ${runLogs[0].summary}`;
  }
  if (key === "water_keeper" && (game.settlement?.water || 0) <= 0 && cores <= 0) {
    lines[2] = "汚染を受けたら浄水フィルタを使って。深部に進む前に体を戻す。";
  }

  return { key: npc.key, name: npc.name, role: npc.role, lines: lines.slice(0, 3) };
}
...
const SettlementSystem = (() => {
  ...
  function generate(game) {
    ...
    game.entranceHintShown = false;
    game.entrancePromptShown = false;
